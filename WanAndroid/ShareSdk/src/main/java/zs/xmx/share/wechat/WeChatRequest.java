...
import zs.xmx.share.R;
import zs.xmx.share.wechat.callbak.IWxLoginCallback;
 ...
 *
 * //todo 待优化封装以及测试
 * 如果要绕过wxEntryActivity,可以看 E:\BaseComponentTest,不过感觉没什么用
 *
 * todo IWxLoginCallback 整合到IOauthLoginCallBack..
 */
public class WeChatRequest {
    private        IWXAPI           mMsgApi;
    private static WeChatRequest    instance;
    private        IWxLoginCallback mIWxLoginCallback = null;
     ...
     *
     * @param context          上下文
     * @param IWxLoginCallback 登录结果回调
     */
    public void login(Context context, IWxLoginCallback IWxLoginCallback) {
        this.mIWxLoginCallback = IWxLoginCallback;
        init(context);
     ...
     * 用于让实现该方法的类传递传输给接口
     */
    public IWxLoginCallback getIWxLoginCallback() {
        return mIWxLoginCallback;
    }
    ...
    private void init(Context context) {
        //todo 改造,外部传进来
        String api_key = "wxa725306b95f9b282";
        //注册微信支付 (上下文,APPID,是否检查签名)
        mMsgApi = WXAPIFactory.createWXAPI(context, api_key, true);
        mMsgApi.registerApp(api_key);
